...
const HERO_PROMPTS = [
  {
    id: 'p-1',
    title: 'Synthesize Cyber-Sigil AI Model',
    model: 'DeepSeek R1 (Reasoning Core)',
    speed: '240 tok/s',
    cost: '$0.00012',
    response: 'Data synthesis complete. Cross-referencing 14,200 cybernetic vectors against x402 settlement ledger. System equilibrium: 99.8%.',
  },
  {
    id: 'p-2',
    title: 'Orchestrate Mecha Workflow',
    model: 'Dolphin 8x7B (Uncensored)',
    speed: '310 tok/s',
    cost: '$0.00008',
    response: 'Autonomous sigil graph deployed. 3 execution nodes verified: Web Scraping -> Entity Extraction -> Vector Memory Lock.',
  },
  {
    id: 'p-3',
    title: 'Verify Zero-Knowledge Cloud Sigil',
...
const SUITE_PILLARS = [
  {
    icon: MessageSquare,
    title: 'Use AI Models',
    tagline: 'Cyber Conversations & Reasoning',
  ...
  },
  {
    ...
    tagline: 'Mecha Studio & Agents',
    description: 'Forge multi-step AI automations, configure autonomous agent personas, and enforce spending caps.',
    to: '/studio',
  ...
  },
  {
    icon: HardDrive,
    title: 'Nexus Cloud',
    tagline: 'Zero-Knowledge Memory & Saves',
    description: 'Store encrypted files, attach knowledge bases to agents, and back up PC game saves with zero-knowledge keys.',
    to: '/cloud',
  ...
  },
  {
    icon: Coins,
    title: 'Manage & Pay',
    tagline: 'Micropayments & Receipts',
    description: 'Track prepaid balances, inspect execution receipts, and manage multi-chain agent budgets in real-time.',
    to: '/payments',
...
const INTEGRATION_PROVIDERS = [
  { name: 'DeepSeek R1', type: 'Reasoning AI Core', status: 'connected' as const },
  { name: 'Dolphin 8x7B Host', type: 'Uncensored Inference', status: 'connected' as const },
  { name: 'Filecoin Network', type: 'Distributed Storage Proof', status: 'connected' as const },
  { name: 'Arweave Blockweave', type: 'Permanent Cloud Storage', status: 'connected' as const },
  { name: 'Base Sepolia x402', type: 'Micro-settlements', status: 'live' as const },
  { name: 'Storj DCS Adapter', type: 'Encrypted Object Storage', status: 'planned' as const },
  { name: 'AWS S3 Adapter', type: 'Zero-K Object Cloud', status: 'demo' as const },
  { name: 'Solana SPL Rail', type: 'High-Speed Settlement', status: 'planned' as const },
];
...
export default function Landing() {
  ...
  return (
    <div className="flex-1 flex flex-col items-center w-full select-none overflow-hidden bg-[#05050A]">
      ...
      <section className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 sm:pt-20 pb-16 sm:pb-24 flex flex-col lg:flex-row items-center justify-between gap-12">
        ...
        <div className="space-y-6 max-w-2xl text-left">
          ...
          <div className="inline-flex items-center gap-2">
            <TruthStateBadge status="connected" text="⚡ NEXUS CYBER-SIGIL v2.0 • MECHA PROTOCOL" />
          </div>

          {/* Main Title */}
          <h1 className="font-display font-extrabold text-3xl sm:text-5xl lg:text-6xl text-white tracking-tight leading-[1.12]">
            Orchestrate <span className="text-prismatic">Cybernetic Digital Capabilities</span> Across the Multiverse.
          </h1>

          {/* Subtitle */}
          <p className="text-sm sm:text-base text-[#A0A5C0] leading-relaxed font-sans max-w-xl">
            Execute AI models, forge autonomous sigil workflows, encrypt zero-knowledge cloud memory, provision GPU compute, and settle micropayments in real-time.
          </p>
        ...
        <div className="w-full max-w-lg shrink-0">
          <div className="p-6 rounded-3xl bg-[#0F0F1D]/90 border border-[#A855F7]/30 backdrop-blur-2xl shadow-[0_0_50px_rgba(168,85,247,0.25)] space-y-5 relative">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-display font-extrabold text-sm text-white flex items-center gap-1.5">
                    <span>Mecha Execution Simulator</span>
                  </h3>
                  <span className="text-[10px] text-[#00F5D4] font-mono">⚡ Test real-time cybernetic actions</span>
                </div>
              </div>
            ...
            <div className="space-y-2">
              <span className="text-[11px] text-zinc-400 font-bold block uppercase tracking-wider font-mono">Select Action Sigil:</span>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      activePrompt.id === prompt.id
                        ...
                        : 'bg-[#05050A] border-white/[0.06] text-zinc-400 hover:text-white hover:border-[#00F5D4]/40'
                    )}
                  >
            ...
            <div className="p-4 rounded-2xl bg-[#05050A] border border-white/[0.08] space-y-3 min-h-[130px] flex flex-col justify-between shadow-inner">
              <div className="space-y-1.5 font-mono text-xs">
                <div className="flex justify-between text-[10px] text-zinc-500 border-b border-white/[0.06] pb-1.5">
                  <span>Model Core: <strong className="text-[#D8B4FE]">{activePrompt.model}</strong></span>
                ...
                <p className={cn('text-zinc-200 text-[11px] leading-relaxed pt-1', isSimulating && 'animate-pulse text-[#00F5D4]')}>
                  {isSimulating ? '⚡ Synthesizing cyber-sigil output and x402 settlement proof...' : activePrompt.response}
                </p>
              </div>
      ...
      {/* ── SUITE LAUNCHER GRID (4 CYBER PILLARS) ── */}
      <section className="w-full bg-[#0A0A14] border-y border-white/[0.08] py-16 sm:py-24">
            ...
            {SUITE_PILLARS.map((pillar) => {
              ...
              return (
                <Link
                  key={pillar.title}
                  to={pillar.to}
                  className="group p-6 rounded-2xl bg-[#0F0F1D] border border-white/[0.08] hover:border-[#00F5D4]/50 transition-all flex flex-col justify-between space-y-6 shadow-xl hover:shadow-[0_0_30px_rgba(0,245,212,0.15)]"
                >
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 space-y-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {INTEGRATION_PROVIDERS.map((item) => (
            <div key={item.name} className="p-4 rounded-xl bg-[#0F0F1D] border border-white/[0.08] flex items-center justify-between gap-3 hover:border-[#A855F7]/40 transition-colors">
