...
export default function TopNav({ onSearchClick, onMobileMenuClick }: TopNavProps) {
  ...
  const navLinks = [
    ...
    { to: '/marketplace/models', label: 'AntSeed Models' },
    { to: '/exchange', label: 'Workflow Exchange' },
    { to: '/balance', label: 'AI Balance' },
    ...(isSignedIn ? [
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-layer-header h-16 px-4 sm:px-6 bg-[#141416]/95 backdrop-blur-xl border-b border-zinc-800/80 flex items-center justify-between select-none shadow-lg">
      <div className="max-w-7xl w-full mx-auto flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <Link to="/" className="flex items-center gap-2.5 group">
            <motion.div whileHover={{ rotate: 8, scale: 1.08 }} whileTap={{ scale: 0.95 }}>
              <NexusLogoMark className="h-7 w-7 sm:h-8 sm:w-8 text-emerald-400" />
            </motion.div>
        ...
        {/* Center: Primary Desktop Navigation Bar (Single Authority, No Side Menu Collision) */}
